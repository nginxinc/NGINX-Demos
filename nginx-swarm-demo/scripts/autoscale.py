...
# Function getDocker
#
# Make a call to the Docker API.
################################################################################
def getDocker(client, swarmMaster, dockerAPIPort, path):

    url = 'http://' + swarmMaster + ':' + str(dockerAPIPort) + path

    try:
        response = client.get(url) # Make an Docker API call
    except requests.exceptions.ConnectionError:
        print("Error: Unable to connect to " + url)
        sys.exit(1)
