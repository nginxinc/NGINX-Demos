# Demo to show NGINX Plus Dynamic Reconfiguration API with etcd
...
4.  Copy `nginx-repo.key` and `nginx-repo.crt` files for your account to `~/NGINX-Demos/etcd-demo/nginxplus`

5.  Move into the etcd-demo directory and start the Vagrant vm:
...
4.  Copy `nginx-repo.key` and `nginx-repo.crt` files for your account to `/srv/NGINX-Demos/etcd-demo/nginxplus`

5.  Move into the etcd-demo directory which contains the demo files and set HOST_IP on line 5 in script.sh to the IP of your Ubuntu VM on which NGINX Plus will be listening.

    `$ cd /srv/NGINX-Demos/etcd-demo`

6.  Run the ansible playbook against localhost on Ubuntu VM:

    `$ sudo ansible-playbook -i "localhost," -c local /srv/NGINX-Demos/etcd-demo/setup_etcd_demo.yml`
...
2.  Copy `nginx-repo.key` and `nginx-repo.crt` files for your account to `~/NGINX-Demos/etcd-demo/nginxplus/`
...
7.  Execute the etcd_exec_watch.sh script in background (This invokes an etcdctl exec-watch command watching for changes in etcd keys and trigger script.sh whenever a change is detected).
